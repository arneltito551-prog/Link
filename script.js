...
const state = {
  ...
  filteredId: null,    // when a single item is focused (from clicking a card)

  searchQuery: ''     // text from view search box
...
function showSection(id) {
  ...
  state.filteredId = null; // clear single-item focus when switching sections
...
qs('#requestForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if(fileInput && fileInput.files && fileInput.files[0]) {
      ...
      showToast('Uploading image...', {type:'info', duration:1200});
...
function computeStatus(i) {
  ...
  if((i.status || '') === 'Approved') return { text:'Approved', class:'approved' };

  if(del >= req && req > 0) return { text:'Completed', class:'completed' };

  if(del > 0 && del < req) return { text:'Partial', class:'pending' };
...
/* =========================
   ...
   Render Requests (with filter + search + single-item focus)
...
function renderRequests(filter='all') {
  ...
  // If single-item filter is set, narrow to that ID
  ...
  if(state.filteredId) {
    ...
    // apply main filter (All / Pending / Partial / Approved / Rejected / Completed)

    if(filter !== 'all') {

      arr = arr.filter(i => {

        if(filter === 'partial') return (i.deliveredQty || 0) > 0 && (i.deliveredQty || 0) < (i.requestedQty || 0);

        if(filter === 'Approved') return (i.status || '') === 'Approved';

        if(filter === 'Rejected') return (i.status || '') === 'Rejected';

        if(filter === 'Pending') return (i.status || '') === 'Pending' && (i.deliveredQty || 0) === 0;
  ...
  if(q !== '') {
    ...
    arr = arr.filter(i => ((i.title||'').toLowerCase().includes(q) || (i.requester||'').toLowerCase().includes(q)));
  ...
  arr.forEach(i => {
    ...
    // store id as data attribute for click handling
    ...
    div.innerHTML = `
      ...
      <div class="row">
        ...
        <div><div class="badge ${st.class.toLowerCase()}">${st.text}</div></div>
  ...
  // show/hide the clear button if filteredId active

  qs('#clearFilterBtn').style.display = state.filteredId ? 'inline-block' : 'none';

}

/* =========================

   Action handlers - delegated on requestsList

   - If click target is a button => perform action

   - Else if clicked inside a .req => focus that single item (no modal)
...
qs('#requestsList').addEventListener('click', async (e) => {
  ...
  // if not a button, check if clicked inside a card (.req)

  const card = e.target.closest('.req');

  if(card && card.dataset && card.dataset.id) {

    // Set filteredId to this card's id and render - do NOT open modal

    state.filteredId = card.dataset.id;
...
/* double-click requests list to clear single-item focus */

qs('#requestsList').addEventListener('dblclick', () => {
...
/* =========================
   ...
   Other list renders (delivered / remaining)
...
/* actions for remaining list (delegated) */
...
/* =========================
   Usage / Reports (no export CSV) - Print only
   ========================= */
...
/* =========================
   Search box wiring (real-time) inside View
   ========================= */
const viewSearch = qs('#viewSearch');
viewSearch.addEventListener('input', () => {
  state.searchQuery = viewSearch.value || '';
  // When typing, clear single-item focus so you see fresh results
  state.filteredId = null;
  renderRequests(state.currentFilter);
});
...
qsa('.filter-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    state.filteredId = null;
    // clear search when hitting a filter? no — keep search but reset single-item focus
    renderRequests(f);
  });
});

/* =========================
   Edit modal (Open button) - unchanged behavior
   ========================= */
...
function updateHomeStats() {
  ...
  const completed = state.items.filter(i => (i.status || '') === 'Completed' || (i.deliveredQty || 0) >= (i.requestedQty || 0)).length;
